...
class sci_method:
    ...
    def is_dark(self, img, threshold=100):
        ...
        gray = cv2.cvtColor(img.numpy(), cv2.COLOR_BGR2GRAY)#转灰度图
    ...
    def process_dark(self, data_path, train_path):
        data_path = glob(os.path.join(data_path, '*.jpg'))  # 递归获取目录下的所有png文件路径
    ...
    def run(self, in_path):
        ...
        TestDataset = MemoryFriendlyLoader(img_dir=in_path, task='test')#数据处理 相当于TensorDataset
...
if __name__ == '__main__':
    data_path = '/home/abc/sci/SCI-main/data/medium'
    sci = sci_method()
    a = sci.run(in_path=data_path)
    print(a)
